...
co = cohere.Client()
def cohere_caller(prompt):
    config_path = CONFIG_DIR / 'talkative-llm' / 'cohere_llm_example.yaml'
    caller = CohereCaller(config=config_path)
    chat_history = [
    {'role': 'system', 'message': 'What is the answer to this joke?'},
    {'role': 'user', 'message': 'What did the janitor say when he jumped out of the closet?'},
    {'role': 'assistant', 'message': 'Supplies!'}]

    results = co.chat(message=prompt,chat_history=chat_history)
    print(prompt)
    print("Answer: ", results.text)
    del caller
...
if __name__=="__main__":
    for prompt in PROMPTS:
        cohere_caller(prompt)
    # openai_caller_completion()
    # test_openai_caller_chat()